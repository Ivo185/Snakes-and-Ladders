...
class FullscreenGameApp:
    def __init__(self, root):
        ...
        self.microbit_value = None
        self._mb_listening  = False
        self.microbit_forced_off = False
        
        self._detect_microbit()
    ...
    def show_scene_game(self):
        ...
        initial_dice_img = self.zar_images[1] if (self.microbit_port and not self.microbit_forced_off) else self.zar_images['натисни']
        self.zar_item = self.canvas.create_image(left_margin_center, int(180 * self.scale), image=initial_dice_img, tags="dice")
        ...
        self.status_txt = self.canvas.create_text(left_margin_center, int(330 * self.scale), text="", font=("Arial", int(40 * self.scale), "bold"), fill="white", justify="center", width=max_text_width)
        
        self.canvas.create_image(right_margin_center, self.height - int(150 * self.scale), image=self.btn_anims['house']['normal'], tags="home_btn")
        self.bind_animated_button("home_btn", self.btn_anims['house'], self.confirm_exit_to_start)
        
        settings_font = max(12, int(15 * self.scale))
        mb_label = "micro:bit: свързан" if (self.microbit_port and not self.microbit_forced_off) else "micro:bit: не е открит"
        mb_color  = "#4CAF50" if (self.microbit_port and not self.microbit_forced_off) else "#f44336"
        
        self.canvas.create_text(right_margin_center, int(50 * self.scale), text=mb_label, fill=mb_color, font=("Arial", settings_font, "bold"), tags="mb_status_txt")
        
        self.canvas.create_text(right_margin_center, int(90 * self.scale), text="⚙ Настройки COM", fill="white", font=("Arial", settings_font, "bold"), tags="settings_btn")
        self.canvas.tag_bind("settings_btn", "<Button-1>", lambda e: self._open_microbit_test())
        self.canvas.tag_bind("settings_btn", "<Enter>", lambda e: (self.canvas.itemconfig("settings_btn", fill="#FFD700"), self.canvas.config(cursor="hand2")))
        self.canvas.tag_bind("settings_btn", "<Leave>", lambda e: (self.canvas.itemconfig("settings_btn", fill="white"), self.canvas.config(cursor="")))
        
        self.canvas.create_text(right_margin_center, int(130 * self.scale), text="↻ Обнови връзката", fill="white", font=("Arial", settings_font, "bold"), tags="refresh_mb_btn")
        self.canvas.tag_bind("refresh_mb_btn", "<Button-1>", lambda e: self._refresh_mb_connection())
        self.canvas.tag_bind("refresh_mb_btn", "<Enter>", lambda e: (self.canvas.itemconfig("refresh_mb_btn", fill="#4FC3F7"), self.canvas.config(cursor="hand2")))
        self.canvas.tag_bind("refresh_mb_btn", "<Leave>", lambda e: (self.canvas.itemconfig("refresh_mb_btn", fill="white"), self.canvas.config(cursor="")))
        
        self.canvas.create_text(right_margin_center, int(170 * self.scale), text="✖ Разкачи micro:bit", fill="white", font=("Arial", settings_font, "bold"), tags="disconnect_mb_btn")
        self.canvas.tag_bind("disconnect_mb_btn", "<Button-1>", lambda e: self._manual_disconnect())
        self.canvas.tag_bind("disconnect_mb_btn", "<Enter>", lambda e: (self.canvas.itemconfig("disconnect_mb_btn", fill="#FF5252"), self.canvas.config(cursor="hand2")))
        self.canvas.tag_bind("disconnect_mb_btn", "<Leave>", lambda e: (self.canvas.itemconfig("disconnect_mb_btn", fill="white"), self.canvas.config(cursor="")))
        
        self.update_status_msg()
        self.draw_players()
        self._auto_poll_microbit()
        

    def _manual_disconnect(self):
        self.microbit_forced_off = True
        self.microbit_port = None
        self.canvas.itemconfig("mb_status_txt", text="micro:bit: изключен", fill="#f44336")
        self.canvas.itemconfig(self.zar_item, image=self.zar_images['натисни'])
        self.update_status_msg()
        messagebox.showinfo("Micro:bit", "Връзката е прекъсната. Използвайте мишката за зара.")


    def update_status_msg(self):
        if self.microbit_port and not self.microbit_forced_off:
    ...
    def draw_players(self):
        ...
        for p_id in pos_groups[0]:
            ...
            self.canvas.create_text(px, label_y, text=f"Играч {p_id}", fill="white", font=("Arial", font_size, "bold"), tags="player_token")
    ...
    def _detect_microbit(self):
        if not HAS_SERIAL: return
        saved_port = None
        config_path = get_external_path("microbit_port.txt")
        if os.path.exists(config_path):
            try:
                with open(config_path, "r", encoding="utf-8") as f: saved_port = f.read().strip()
            except Exception: pass
            
        try:
            ports = serial.tools.list_ports.comports()
            self.microbit_port = None
            if saved_port and any(p.device == saved_port for p in ports): self.microbit_port = saved_port
            else:
                for p in ports:
                    desc = (p.description or "").lower()
                    if "microbit" in desc or "mbed" in desc or "arm" in desc:
                        self.microbit_port = p.device
                        break
        except Exception: self.microbit_port = None
        
        if self.microbit_port:
            try:
                test_ser = serial.Serial(self.microbit_port, 115200, timeout=1)
                test_ser.close()
                self._mb_listening = True
                threading.Thread(target=self._mb_listener, daemon=True).start()
            except Exception: self.microbit_port = None


    def _refresh_mb_connection(self):
        self.microbit_forced_off = False
        self._mb_listening = False
        self.canvas.itemconfig("mb_status_txt", text="Зареждане...", fill="#FFD700")
        
        def finish_refresh():
            self._detect_microbit()
            mb_label = "micro:bit: свързан" if self.microbit_port else "micro:bit: не е открит"
            mb_color  = "#4CAF50" if self.microbit_port else "#f44336"
            self.canvas.itemconfig("mb_status_txt", text=mb_label, fill=mb_color)
            if getattr(self, 'current_scene', None) == "game":
                current_img = self.zar_images[1] if self.microbit_port else self.zar_images['натисни']
                self.canvas.itemconfig(self.zar_item, image=current_img)
                self.update_status_msg()
                
        self.root.after(1200, finish_refresh)


    def _mb_listener(self):
        try:
            with serial.Serial(self.microbit_port, 115200, timeout=1) as ser:
                while self._mb_listening:
                    try:
                        line = ser.readline().decode("utf-8").strip()
                        if line.isdigit() and 1 <= int(line) <= 6: self.microbit_value = int(line)
                    except Exception: pass
        except Exception: self.microbit_port = None


    def _auto_poll_microbit(self):
        if getattr(self, 'current_scene', None) != "game": return
        
        if self.microbit_port and not self.microbit_forced_off and not self.is_moving and self.microbit_value is not None:
            val, self.microbit_value = self.microbit_value, None
            self.is_moving = True
            if PYGAME_AVAILABLE:
                try: pygame.mixer.Sound(resource_path("sounds/dice_sound.wav")).play()
                except: pass
            self._animate_then_roll(val, 0)
            
        self.root.after(100, self._auto_poll_microbit)
    ...
    def roll_dice(self):
        ...
        if self.microbit_port and not self.microbit_forced_off: return
    ...
    def next_turn(self):
        ...
        self.update_status_msg()
        next_zar_img = self.zar_images[1] if (self.microbit_port and not self.microbit_forced_off) else self.zar_images['натисни']
        self.canvas.itemconfig(self.zar_item, image=next_zar_img)
    ...
    def _open_microbit_test(self):
        script = get_external_path("microbit_test.py")
        if os.path.exists(script): subprocess.Popen([sys.executable, script])
